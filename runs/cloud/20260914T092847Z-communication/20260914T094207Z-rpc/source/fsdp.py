...
class _Shard:
    ...
    def range(self, operation):
        return torch.cuda.nvtx.range(f"fsdp_{operation}:{self.name}") if self.parameter.is_cuda else nullcontext()
    ...
    def reduce_gradient(self, gradient):
        ...
        with self.range("reduce_scatter"):
...
class _ShardedLinear(torch.autograd.Function):
    ...
    def forward(ctx, x, weight, bias, shard):
        ...
        with shard.range("linear_forward"):
            output = xc @ wc.T
        if bias is not None:
...
class FSDP(torch.nn.Module):
    def __init__(self, module, compute_dtype=None):
        ...
        module_names = {child: name for name, child in module.named_modules()}

...
def model_benchmark(cfg, out):
    ...
    opt = torch.optim.AdamW(model.parameters(), lr=1e-3) if cfg["mode"] == "train" else None
    if cfg.get("compile"):
    ...
    def step(record=False):
        ...
        with torch.autocast("cuda", dtype=dtype, enabled=dtype != torch.float32):
            with torch.cuda.nvtx.range("forward"):
                logits = model(x)
                # Match language-model training: stable FP32 cross entropy.
    ...
    try:
        result = timed(step, cfg.get("warmup", 5), cfg.get("steps", 10))
        model.zero_grad(set_to_none=True)
        sync()
        torch.cuda.reset_peak_memory_stats()
        loss = step(record=True)
        sync()
        result.update({"last_loss": loss.item(), "parameters": parameter_count, "initial_memory": initial,
    ...
    for dtype in (torch.float32, torch.bfloat16):
        for n, d in ((32, 16), (128, 64), (256, 128), (63, 32)):
            for causal in (False, True):
                ...
                refs = [v.float().requires_grad_() for v in base]
                ...
                for cls in (FlashAttentionTriton, FlashAttentionTritonFull):
                    inputs = [v.clone().requires_grad_() for v in base]
...
def distributed_worker(rank, world, cfg, output_dir, port):
    ...
    try:
        if cfg["kind"] == "allreduce":
            ...
            opt = (ShardedOptimizer(wrapper.parameters(), torch.optim.AdamW, lr=1e-3)
                   if cfg.get("optimizer") == "sharded" else torch.optim.AdamW(wrapper.parameters(), lr=1e-3))

...
class FlashAttentionPyTorch(torch.autograd.Function):
    ...
    def backward(ctx, do):
        ...
        gradients = (*run_backward(q, k, v, o, do, l, ctx.is_causal), None)
...
class FlashAttentionTritonFull(FlashAttentionTriton):
    ...
    def backward(ctx, do):
        ...
        q, k, v, o, l = ctx.saved_tensors
        gradients = (*backward(q, k, v, o, do, l, ctx.is_causal), None)
        return gradients[:len(ctx.needs_input_grad)]
...
class FlashAttentionTritonTuned(torch.autograd.Function):
    ...
    def backward(ctx, do):
        from .triton_attention import backward
        q, k, v, o, l = ctx.saved_tensors
        gradients = (*backward(q, k, v, o, do, l, ctx.is_causal, tile=64), None)
        return gradients[:len(ctx.needs_input_grad)]
